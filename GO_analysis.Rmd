...
gene2GO <- go %>%
  ...
  filter(GO_terms != "") %>% # this somehow makes GO result in less...
...
#---------input DE gene----------#
load("~/Box/Bioinfo/ArabidopsisSubmergence/DGEs.RData")
DGEs %>% 
  lapply(function(de){
...
#--------------------------------#
mapdata_all <- heatData  %>% 
  dplyr::select(-GO_term,-Description,-expression,-ontology) %>% 
  set_rownames(seq(1:length(.[[1]])))

gad2_1245 <- heatData %>% 
  dplyr::select(gad21,gad1245,gad1245_special,gad1SALK) %>% 
  filter(gad21 != 0 | gad1245 != 0 | gad1245_special != 0 | gad1SALK != 0) %>% 
  set_rownames(seq(1:length(.[[1]])))
#--------------------------------#

Annos <- heatData %>%
  filter(gad21 != 0 | gad1245 != 0 | gad1245_special != 0 | gad1SALK != 0) %>% # if use gad2_1245
  dplyr::select(ontology) %>% 
  set_rownames(seq(1:length(.$ontology)))

pheatmap::pheatmap(mat = gad2_1245,
                   cluster_cols = FALSE,
