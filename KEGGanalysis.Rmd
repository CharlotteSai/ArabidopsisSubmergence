...
# all KOI are marked in my goodnotes
```
...
keggColor <- KEGGheat[["gtable"]][["grobs"]][[2]][["children"]][["GRID.rect.102"]][["gp"]][["fill"]]
keggColor %<>% 
  as.data.frame %>% 
...
# # gad1245_special
# keggFinal$gad1245_special %>% 
#   select(Description,keggID) %>%
#   mutate(Class = keggGet(keggID)[[1]]$CLASS) %>% 
#   left_join(keggColor, by = "keggID") %>% 
#   write_csv("kegg2gad1245Special.csv")
```
...
pathview(gene.data = mapk_exp, 
         ...
         bins = list(gene = 11),
         kegg.dir = "~/Box/Bioinfo/ArabidopsisSubmergence/pathview",
         kegg.native = TRUE,
         width = 70)
```
...
ath04075Exp <- DGEs %>%
  lapply(function(de){
    de %>% 
      filter(GeneID %in% 
               intersect(pathway2gene$ath04075,
                         de$GeneID)) %>% 
      select(GeneID, logFC, comparison)
  }) %>% 
  bind_rows() %>% 
...
pathview(gene.data = ath04075Exp, 
         gene.idtype = "KEGG",
         pathway.id = "04075", 
         species = "ath",
         na.col="transparent",
